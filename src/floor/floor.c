...
{
	data->floor.floor_img.img_ptr = mlx_xpm_file_to_image(data->mlx_connection, "assets/floor/floor.xpm", &data->floor.floor_img.width,&data->floor.floor_img.height);
	data->floor.floor_img.img_pixels_ptr = mlx_get_data_addr(data->floor.floor_img.img_ptr, &(data->floor.floor_img.bits_per_pixel), &(data->floor.floor_img.size_line), &(data->floor.floor_img.endian));
...
void	loop_verticaly(t_data *data, t_player *player, t_floor *floor, int i)
{
	...
	floor->floor_step_x = floor->row_distance * (floor->floor_dir_x1 - floor->floor_dir_x0) / WIDTH;
	floor->floor_step_y = floor->row_distance * (floor->floor_dir_y1 - floor->floor_dir_y0) / WIDTH;
...
void	loop_horizontaly(t_data *data, t_floor *floor, int i, int j)
{
	...
	floor->tx = (int)(TEXWIDTH * (floor->floor_x - floor->cell_x)) & (TEXWIDTH - 1);
	floor->ty = (int)(TEXHEIGHT * (floor->floor_y - floor->cell_y)) & (TEXHEIGHT - 1);
	floor->floor_x += floor->floor_step_x;
	floor->floor_y += floor->floor_step_y;
	pixel_index = (floor->ty * floor->floor_img.size_line) + (floor->tx * (floor->floor_img.bits_per_pixel / 8));
