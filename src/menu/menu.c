...
{
    if (keysm == 1 && !data->menu.menu && !data->menu.pause && !data->menu.controls && !data->menu.change)
    {
        data->player.is_firing = 1;
		data->player.fire_frame = 0;
    }
    else if (keysm == 1 && data->menu.menu && !data->menu.pause && !data->menu.controls && (x > 375 && x < 900) && (y > 140 && y < 230))
        data->menu.menu = 0;
